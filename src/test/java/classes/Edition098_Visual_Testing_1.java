...
public class Edition098_Visual_Testing_1 {
    ...
    private final static String VALIDATION_PATH = "/home/guray/IdeaProjects/appium-cucumber-testng/src/images";

    final static String CHECK_HOME = "home_screen";
    ...
    public  Edition098_Visual_Testing_1(AndroidDriver<WebElement> driver)
    {
        this.driver=driver;
    }
...
//    }
    ...
    WebElement waitForElement(WebDriverWait wait, By selector) {
    ...
    static void doVisualCheck(String checkName) throws Exception {
       ...
       // SimilarityMatchingResult res = driver.getImagesSimilarity(baselineImg, driver.getScreenshotAs(OutputType.FILE), opts);
        FeaturesMatchingResult res2 = driver.matchImagesFeatures(baselineImg,driver.getScreenshotAs(OutputType.FILE));

        if(res2.getCount()>0)
        {
            System.out.println("esleşen deger"+res2.getCount());
            System.out.println(""+res2.getRect1().x+"-"+res2.getRect1().y);
            int xPoint=res2.getRect1().x;
            int yPoint=res2.getRect1().y;
            TouchAction touchAction=new TouchAction(driver);
            touchAction.tap(PointOption.point(new Point(xPoint, yPoint))).perform();
        }
